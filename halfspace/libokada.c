...
Vector get_displacement( OkadaSource *src, Vector *where, double poisson ) {
  const Angle dip = from_deg( src->dip );
  const Angle perp_strike = from_deg( src->strike - 90. );
  Vector U = { src->strikeslip / ( 2. * M_PI ), src->dipslip / ( 2. * M_PI ), src->opening / ( 2. * M_PI ) };
  Vector point = { where->x - src->xbottom, where->y - src->ybottom, where->z };
  OkadaConsts consts;
  int i;
  Vector disp = { 0 };
  if ( point.z == 0 ) {
    ...
    for ( i = 0; i < 4; i++ ) {
      okada_get_consts( &consts, point, -src->zbottom, dip, src->length, src->width, i, poisson );
...
Matrix get_gradient( OkadaSource *src, Vector *where, double poisson ) {
  const Angle dip = from_deg( src->dip );
  const Angle perp_strike = from_deg( src->strike - 90. );
  Vector U = { src->strikeslip / ( 2. * M_PI ), src->dipslip / ( 2. * M_PI ), src->opening / ( 2. * M_PI ) };
  Vector point = { where->x - src->xbottom, where->y - src->ybottom, where->z };
  ...
  if ( point.z == 0 ) {
    ...
    Vector_irot_xy( &point, perp_strike );
    for ( i = 0; i < 4; i++ ) {
      okada_get_more_consts( &consts, point, -src->zbottom, dip, src->length, src->width, i, poisson );
      if ( i & 1 ) Vector_flip( &U );
...
void get_displacements( Vector *out, OkadaSource *src, Vector *where, double poisson, int count ) {
  while ( count-- ) Vector_copy( out++, get_displacement(src,where++,poisson) );
}
void get_gradients( Matrix *out, OkadaSource *src, Vector *where, double poisson, int count ) {
  while ( count-- ) Matrix_copy( out++, get_gradient(src,where++,poisson) );
}
